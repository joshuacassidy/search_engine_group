...
public class SearchIndex {
    ...
    public SearchIndex(String indexPath, Analyzer analyzer, Similarity similarity,
        ...
        this.useDoc2Vec = useDoc2Vec;

        if(useDoc2Vec) {
            System.out.println("Using Doc2Vec");
        }

        if(useWordFrequencies) {
            System.out.println("Using Word Frequencies");
        }
    }
    ...
    private void executeQuery(FileWriter resultsFileWriter, IndexSearcher indexSearcher, QueryParser queryParser, String title, String body, String queryNumber) throws Exception {
        ...
        BooleanQuery booleanQuery = new BooleanQuery.Builder()
                .add(new BoostQuery(titleQuery, 5.2f), BooleanClause.Occur.SHOULD)
        ...
        if(useWordFrequencies) {
            List<Float> zipfScores = pythonAPIManager.scoreTextsWithZipf(title + body, texts);
            for(int i = 0; i < Math.min(results.totalHits.value, resultsToConsider); i++) {
                org.apache.lucene.document.Document doc = documents.get(i);
                float zipfScore = zipfScores.get(i);

                // 10 is the usual average of the sqrt(score), we only care about the ones that are over the average
                // so that we will catch documents with rare words that are very significant for the ranking
                float sqrtScore = Math.max((float) (Math.log(zipfScore) - 10), 0);

                if(sqrtScore > 0) {
                    scores.put(doc, 1.13f * scores.get(doc));
                }
            }
        }
